...
public class SnackManager implements TableManager {

    // Statement class is imported from JDBC - an object for our MySQL queries
    static Statement stmt;
    JDBC_Setup jdbc;

    public SnackManager(JDBC_Setup jdbc) {
        this.jdbc = jdbc;
    }
    ...
    public Entry createEntry() {
        ...
        System.out.println("Type com.dimmbar.Entry here in the following format: \n" +
                "name, price, subtype");
    ...
    public void deleteEntry(int entry_id, String col_name) {
        try (Connection conn = jdbc.connect()) {
            ...
            PreparedStatement preStmnt = conn.prepareStatement("DELETE FROM Snacks where " + col_name + " = ?");
            preStmnt.setInt(1, entry_id);

            // execute the statement
            preStmnt.executeUpdate();
            System.out.println("Deletion of com.dimmbar.Entry matching ID: " + entry_id + " successful");
        } catch (Exception e) {
    ...
    public void deleteBySubtype(String value) {
        try (Connection conn = jdbc.connect()) {
            ...
            // prepared statements use SQL ? syntax
            // ? relates to a variable we can pass in
            PreparedStatement preStmnt = conn.prepareStatement("DELETE FROM Snacks where subtype = ?");
            preStmnt.setString(1, value);

            // execute the statement
            preStmnt.executeUpdate();
            System.out.println("Deletion of all Entries matching " + value + " successful");
        } catch (Exception e) {
    ...
    public void deleteAll() {
        try (Connection conn = jdbc.connect()) {
            ...
            // prepared statements use SQL ? syntax
            // ? relates to a variable we can pass in
            PreparedStatement preStmnt = conn.prepareStatement("DELETE FROM Snacks WHERE snack_id > 0;");
    ...
    public Entry viewEntry(int entry_id) {
        try (Connection conn = jdbc.connect()) {
            ...
            stmt = conn.createStatement();
            String query = String.format("SELECT * FROM Snacks WHERE snack_id = %d", entry_id);
    ...
    public ArrayList<Entry> viewAll() {
        try (Connection conn = jdbc.connect()) {
            ...
            stmt = conn.createStatement();
            ArrayList<Entry> Snacks = new ArrayList<>();
            String query = "SELECT * FROM Snacks";
            ResultSet result = stmt.executeQuery(query);
            // our result is a spreadsheet from the database, the first row being meta data
            // go down to the next row of data (our useful data)
            while (result.next()) {
                Snacks.add(jdbc.returnResults(result));
            }
            // pass in the useful data to our returnResults method, return this
            return Snacks;
...
//            System.out.println("results successfully retrieved");
    ...
    public ArrayList<Entry> viewAllWhere(String condition) {
        try (Connection conn = jdbc.connect()) {
            ...
            String query = "SELECT * FROM Snacks WHERE " + condition + ";";
            ResultSet result = stmt.executeQuery(query);
            // our result is a spreadsheet from the database, the first row being meta data
            // go down to the next row of data (our useful data)
            while (result.next()) {
    ...
    public ArrayList<Entry> updateByString(Entry entry, String condition, String value) {
        ...
        try (Connection conn = jdbc.connect()) {
            ...
            String update = String.format("UPDATE Snacks SET " +
                    "name_ = ?, " +
                    "price = ?, " +
                    "subtype = ? " +
                    "WHERE %s = ?;", condition);
